...
#include "size_lru_eviction.h"

using namespace std;

SizeLRUEviction::SizeLRUEviction(unsigned long long size, string id, const EmConfItems * sci){
    name = "size_lru";
    ...
    total_capacity = size;
    cache_id = id;
    head = new SizeLRUEvictionEntry;
    tail = new SizeLRUEvictionEntry;
    head->prev = NULL;
    head->next = tail;
    tail->next = NULL;
    tail->prev = head;
...
SizeLRUEviction::~SizeLRUEviction()
{
    delete head;
    delete tail;
}
 ...
 */
void SizeLRUEviction::print_cache_file_age_histogram() {
    ...
    SizeLRUEvictionEntry *currentNode = head;
 ...
 */
void SizeLRUEviction::print_cache_file_hits() {
 ...
 *
 */
void SizeLRUEviction::hourly_purging(unsigned long timestamp) {
    ...
    // JUNK removal
    // its been X hours. remove small old files from the tail
    if (cache_filled_once == true && total_hourly_purge_intervals >= regular_purge_interval) {
        // NOTE: As long as cistomer overrides are off, this should be too,
        // saves time
        //compute_periodic_stats(sci->floor_customer_loss);
        total_hourly_purge_intervals = 0;
        total_junk_purge_operations++;

        // algo 2
        // regular purge
        while (current_size > total_capacity *.80) {
            total_junk_items_purged++;
            purge_regular();
        }

        cout << endl << "total_junk_items_purged " << total_junk_items_purged << endl;
        total_junk_items_purged = 0;
    }

    total_items_purged = 0;
    cout << endl << current_size << " " << total_capacity << endl;
    while (current_size > total_capacity * .80) {
        total_items_purged++;
        purge_size_based_multimap();
        //	purge_size_based(); // not used anymore, was too slow
    }
    cout << endl << "purge_size_based_call_count " << total_items_purged << endl;
}

// for debugging, also keeps track of count.
unsigned long long SizeLRUEviction::initial_put_count(string key, unsigned long data, unsigned long timestamp, unsigned long bytes_out, string customer_id, string orig_url)
{
    SizeLRUEvictionEntry* node = _mapping[key];
...
unsigned long long SizeLRUEviction::initial_put(string key, unsigned long data, unsigned long timestamp, unsigned long bytes_out, string customer_id, string orig_url, string access_log_entry_string)
{
    SizeLRUEvictionEntry* node = _mapping[key];
    if(node)
    {
...
void SizeLRUEviction::print_oldest_file_age(unsigned long timestamp, ostream &output) {
...
void SizeLRUEviction::print_oldest_file_age_for_monitored_customers(unsigned long currentTimeStamp) {
    ...
    for(vector<string>::const_iterator i = sci->monitor_customers_list.begin(); i != sci->monitor_customers_list.end(); ++i) {
        SizeLRUEvictionEntry* currentNode = tail;
...
unsigned long long SizeLRUEviction::put(string key, item_packet* ip_inst, string orig_url)
{
    SizeLRUEvictionEntry* node = _mapping[key];
    if(node)
    {
...
void SizeLRUEviction::print_avg_oldest_requested_file(unsigned long timestamp) {
...
unsigned long SizeLRUEviction::get(string key, unsigned long ts, unsigned long bytes_out, string url_original)
{
    SizeLRUEvictionEntry* node = _mapping[key];
    if(node)
    {
...
int SizeLRUEviction::check_and_print(string key)	// to check if present.
{
    SizeLRUEvictionEntry* node = _mapping[key];
    if(node) {
        /*	cout
...
int SizeLRUEviction::check(string key, unsigned long ts)	// to check if present.
{
    SizeLRUEvictionEntry* node = _mapping[key];
    if(node)
        return 1;
...
unsigned long SizeLRUEviction::manual_delete(string key) {
    SizeLRUEvictionEntry* node = _mapping[key];
    unsigned long data;
 ...
 */
bool SizeLRUEviction::purge_regular() {
    SizeLRUEvictionEntry* node = tail->prev;	//switch 1 of 2; for LRU vs. MRU; LRU(tail->prev); 		MRU(head->next)
    ...
    // SizeLRUEvictionEntry* headNode = head->next;
...
void SizeLRUEviction::dump_cache_contents(string filename) {
    ofstream myfile;
...
void SizeLRUEviction::dump_cache_contents_cout() {
...
unsigned long long SizeLRUEviction::get_size() {
    return current_size;
}

unsigned long long SizeLRUEviction::get_total_capacity() {
    return total_capacity;
}

void SizeLRUEviction::print_max_cache_item_count() {
...
void SizeLRUEviction::set_total_capacity_by_value(unsigned long long size) {
    total_capacity = size;
}

void SizeLRUEviction::decide_items_based_on_score() {
    compute_scores();
    ...
    for(std::vector<std::pair<double, string> >::reverse_iterator it = score_vector.rbegin(); it != score_vector.rend(); ++it) {
        if(current_size > total_capacity) {
            SizeLRUEvictionEntry* node = _mapping[it->second];
            if (sci->debug) {
...
void SizeLRUEviction::update_size_running_mean(SizeLRUEvictionEntry* node) {
...
void SizeLRUEviction::compute_scores() {
    ...
    for (currentNode = currentNode->prev; currentNode != head; currentNode = currentNode->prev) {
        ...
        else if (3 == sci->eviction_formula) {
            eviction_score = (head->next->timestamp - currentNode->timestamp) * (size_score * w_size); // higher score = evicted sooner
        }

        else if (4 == sci->eviction_formula) {
            // A_i^y * (S_i * w + E)
            eviction_score = pow((head->next->timestamp - currentNode->timestamp), sci->ef4_y)
                * ((size_score * w_size) + sci->ef4_e); // higher score = evicted sooner
        }
        else if (5 == sci->eviction_formula) {
            // A_i^y * (S_i * w + A_i)
            eviction_score = pow((head->next->timestamp - currentNode->timestamp), sci->ef4_y)
                * ((size_score * w_size) + (head->next->timestamp - currentNode->timestamp)); // higher score = evicted sooner
        }

        else if (6 == sci->eviction_formula) {
            // A_i^y + (S_i * w * A_i)
            eviction_score = pow((head->next->timestamp - currentNode->timestamp), sci->ef4_y)
                + ((size_score * w_size) * (head->next->timestamp - currentNode->timestamp)); // higher score = evicted sooner
        }

        else if (7 == sci->eviction_formula) {
            // A_i^y  * (w_s * A_0 * S + e)
            eviction_score = pow((head->next->timestamp - currentNode->timestamp), sci->ef4_y)
                * ((size_score * w_size * (head->next->timestamp - tail->prev->timestamp)) + sci->ef4_e); // higher score = evicted sooner
        }

        else if (8 == sci->eviction_formula) {
            // same as eviction_formula 1, but every lru_interval hours we do a regular LRU
            if (0 == hour_count % sci->lru_interval) {
...
void SizeLRUEviction::update_cost_based_score(SizeLRUEvictionEntry* node) {
...
void SizeLRUEviction::detach(SizeLRUEvictionEntry* node)
{
...
void SizeLRUEviction::attach(SizeLRUEvictionEntry* node)
{
    node->next = head->next;
    node->prev = head;
    head->next = node;
...
void SizeLRUEviction::purge_size_based_multimap() {
    ...
    SizeLRUEvictionEntry* node = tail->prev;
    ...
    while (current_size > total_capacity) {
        ...
        SizeLRUEvictionEntry* node_to_delete = _mapping[it->second];
...
string SizeLRUEviction::return_customer_id(string url) {
...
void SizeLRUEviction::compute_periodic_stats(bool floor_customer_loss) {
...
void SizeLRUEviction::periodic_output(unsigned long ts, std::ostringstream& outlogfile){
